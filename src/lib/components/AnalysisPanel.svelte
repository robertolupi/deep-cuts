<script lang="ts">
  ...
  const PASS_META: Record<string, { label: string; description: string }> = {
    ...
    boundary_refine:   { label: 'Boundary Refinement',    description: 'Augments structure edges with strong novelty peaks from the energy envelope' },
...
<style>
  ...
  .file-status { font-family: var(--sg-font-mono); font-size: var(--sg-text-2xs); }
