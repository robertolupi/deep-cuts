...
| Listing a no-`track_id` table in `owned_tables` | `reset_pass` fails with "no such column: track_id" | Use `owned_tables: &[]` + `custom_reset: Some(...)` to delete the table manually; add a dedicated reset test for the pass |
...
| Adding a frontend-visible field without updating TypeScript types | Frontend silently receives `undefined`; type errors only surface at runtime | Follow step 5 — add to `types.ts`, `CommandMap`, local-debug mocks in `src/lib/ipc.ts`, **and** `src/lib/mock-data.ts` |
...
| Batch pass not logging metrics with `run_id` | Pass latency is invisible in the Metrics Inspector; runs cannot be correlated | Call `crate::metrics_database::log_system_event` at the end of `execute()` with the pass name and summary; if metrics genuinely do not apply, document why in a comment |
