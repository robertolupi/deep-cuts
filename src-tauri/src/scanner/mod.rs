...
pub async fn scan_all_libraries(
    ...
    // Spawns the synchronous, blocking scanner pipeline in a thread pool task
    // to keep the frontend and async executors fully non-blocking and responsive.
    tauri::async_runtime::spawn_blocking(move || {
        ...
        if total_files == 0 {
            ...
            for (dir_id, files) in &directory_file_maps {
                ...
                let _ = db::reconcile_deleted_tracks(&mut conn, *dir_id, &active_paths);
            }
        ...
        for (dir_id, files) in &directory_file_maps {
            let active_paths: HashSet<String> = files.iter().map(|f| f.path.clone()).collect();
            if let Err(e) = db::reconcile_deleted_tracks(&mut conn, *dir_id, &active_paths) {
