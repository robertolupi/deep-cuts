...
pub fn run_clap_phase(app: &tauri::AppHandle, conn_arc: &Arc<Mutex<Connection>>) {
    ...
    let clap_pending: Vec<ClapSpoolJob> = {
        let conn = match super::lock_analysis_conn(conn_arc, "clap") {
            Ok(conn) => conn,
            Err(e) => {
                super::emit_pipeline_error(app, "clap", e);
                let _ = app.emit("analysis-complete", ());
                return;
            }
        };
