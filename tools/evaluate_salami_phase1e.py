#!/usr/bin/env python3
"""Phase 1e Evaluation: Hybrid Supervised Classifier + DP Decoder.

Trains a Random Forest boundary probability estimator on candidate points 
(baseline + refined + SSM peaks) using the Inner Dev fold (80%), then feeds 
these probabilities as the local reward to a global 1-D segment-path DP decoder 
that optimizes boundary count, spacing, and log-normal duration priors.
"""
...
from evaluate_salami_phase1c import (
    extract_track_candidates_and_features,
)
...
def run_hybrid_dp_decoder(
    ...
    """Runs segment-path DP over candidates using classifier probabilities as the local reward."""
    duration = float(track["duration"])
    
    if not candidates or probs.size == 0:
    ...
    for k in range(1, K + 2):
        for j in range(k):
            ...
            max_lp = -mu + (dur_sigma ** 2) / 2.0 - np.log(dur_sigma * np.sqrt(2.0 * np.pi))
            log_prob_normalized = log_prob - max_lp
            dur_cost = -weight_dur * log_prob_normalized
            
            # Reward for boundary placement (class 1 probability log-odds)
            reward = 0.0
            if k < K + 1:
                p = float(probs[k - 1])
                p = min(max(p, 1e-4), 1.0 - 1e-4)
                log_odds = np.log(p / (1.0 - p))
                reward = log_odds - lambda_val
...
def optimize_hybrid_hyperparameters(
    tracks: list[dict],
    mode: str,
    window: float,
    objective_type: str,
) -> tuple[dict[str, Any], int]:
    """Tunes the DP parameters on top of the classifier probabilities."""
    print(f"\nRunning DP HPO on Classifier Probabilities using objective={objective_type}...")
    best_score = 0.0
    best_params = {}
    n_configs = 0
    
    # Space search:
    # lambda_val: 0.0 to 1.0 (since probabilities are in [0, 1])
    # target_dur: 15.0 to 35.0s
    # dur_sigma: 0.3 to 1.2
    # weight_dur: 0.1 to 2.5
    # min_gap: 3.0 to 7.0s
    
    rng = np.random.default_rng(42)
    trials = 80  # Reduced to stay within the <= 150 budget
    
    for trial in range(trials):
        lam = float(rng.uniform(-4.0, 1.0))
    ...
    res = minimize(
        ...
        initial_guess,
        method="Nelder-Mead",
        options={"maxiter": 25, "disp": False}  # Reduced to stay within the <= 150 budget
    )
    
    opt_lam, opt_tgt, opt_sig, opt_w = res.x
    opt_lam = clip(opt_lam, -6.0, 2.0)
    opt_tgt = clip(opt_tgt, 10.0, 45.0)
    opt_sig = clip(opt_sig, 0.1, 2.0)
    opt_w = clip(opt_w, 0.01, 5.0)
    
    opt_score = evaluate_hybrid_config(
...
def run_hybrid_evaluation_split(
    tracks: list[dict],
    ...
    """Runs final evaluation and computes statistics for the Hybrid model on a subset of tracks."""
    evaluated = []
    for track in tracks:
        duration = float(track["duration"])
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Phase 1e Hybrid RF+DP boundary evaluation.")
    ...
    print("\nExtracting candidates and feature vectors...")
    ...
    # 3. Train Classifier
    print("Training Random Forest Classifier on Inner Dev Fold...")
    clf = RandomForestClassifier(n_estimators=100, max_depth=6, min_samples_leaf=4, random_state=42)
    clf.fit(X_train, y_train)
    
    # Compute predicted probabilities for each track's candidates
    print("Calculating candidate probability scores...")
    for track in tracks:
    ...
    # 4. HPO for DP parameters on Inner Dev Fold
    best_params, n_configs_evaluated = optimize_hybrid_hyperparameters(
