...
Each tag is in the format "namespace:label". Tags prefixed with "clap:" were generated
by audio embedding similarity; all others were generated by a language model.
...
def load_track_data(audio_path: Path) -> dict | None:
    ...
    try:
        ...
        row = conn.execute(
            """SELECT id, ai_mood, ai_instruments, description
        ...
        return {"error": str(e), "qwen_tags": [], "clap_tags": [], "all_tags": []}
...
def validate_song(song_dir: Path, client: genai.Client) -> dict:
    ...
    all_tags  = db_data.get("all_tags",  []) if db_data else []
    qwen_tags = db_data.get("qwen_tags", []) if db_data else []
    clap_tags = db_data.get("clap_tags", []) if db_data else []
    ...
    if all_tags:
        ...
        result["tag_validation"] = {"skipped": "no tags in DB"}
...
def main():
    ...
    print(f"  {ok} succeeded, {errors} failed, {tag_skipped} had no tags to validate.")

    # Accuracy by source
    from collections import Counter
    correct_by_source: Counter = Counter()
    ...
    for src in ["qwen", "clap"]:
        ...
        if t:
            print(f"  {src:6s} accuracy: {c}/{t} ({100*c//t}%)")
